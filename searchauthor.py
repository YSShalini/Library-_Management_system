...
def search_book():
    author_name = entry_book_name.get()
    ...
    try:
        ...
        # Execute the query to search for all books by the author
        cursor.execute("SELECT * FROM books WHERE author = %s", (author_name,))
        books_details = cursor.fetchall()  # Fetch all results
        
        if books_details:
            # If books found, display their details
            book_info = ""
            for book_details in books_details:
                book_info += f"Book ID: {book_details[7]}\nName: {book_details[0]}\nAuthor: {book_details[1]}\n\n"
            book_info_label.config(text=book_info)
        else:
            # If books not found, display "No record found"
...
Label(window, text='', bg='light blue', height=25, width=200).place(x=60, y=140)
Label(window, text='ENTER AUTHOR NAME', font=f).place(x=610, y=200)
